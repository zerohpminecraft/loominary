...
The **error-diffusion family** pushes each pixel's rounding error onto its neighbors; the algorithms differ in how far and how evenly the error spreads:
...
Every error-diffusion algorithm gets a **strength slider (0.1–1.0)**; lower values diffuse less error, giving cleaner flats at the cost of banding. There is also a **serpentine scan** checkbox that alternates scan direction per row to break up directional drift; try it if you see diagonal "worms".

**Ordered:** `Bayer` uses a fixed threshold matrix instead of error diffusion, producing a regular crosshatch pattern that compresses well and reads as deliberate texture. Its options are **Bayer scale** (0.02–0.20, pattern contrast) and **matrix size** (2×2 / 4×4 / 8×8 / 16×16; smaller = coarser pattern).

`None`, the **default**, disables dithering: every pixel snaps to its single nearest palette color. It's the right choice for logos, flat-color art, and [animations](Animated-Art) (dither noise is what video codecs handle worst); switch to an error-diffusion algorithm when gradients start banding.

### Adaptive strength

With FS selected at import, Loominary computes a per-pixel dither strength map from local image detail: smooth gradients dither fully, sharp edges and flat fills stay clean. In the editor you can view this mask (**M**) and paint it manually with the dither brush; see [Editor Tools](Editor-Tools).
...
Fewer colors also means smaller payloads, so a restricted palette doubles as a [budget](Codecs-and-Capacity) tool on top of being an aesthetic choice.

## The match-quality score

Above the preview, **"Palette coverage: N%"** reports the fraction of pixels whose nearest palette color lands within a perceptual ΔE of 0.05 (green ≥75%, amber ≥50%, red below), along with the average ΔE. It measures palette suitability only (dithering and chroma boost don't move it), which makes it a reliable way to compare palette choices and adjustments objectively.
...
3. If gradients band, turn dithering on (`FS`); if they still band, try JJN. If flats get noisy, lower the strength or try Atkinson.
4. If colors feel muddy, set chroma boost to 1.25 or switch the metric to `Chr+`.
5. For animations, keep dithering at `None`, use fewer colors, and let [lossy AV1](Animated-Art) handle the gradients.
