...
Brush strokes are interpolated so fast drags leave no gaps, and the fill/wand tolerance is perceptual (OKLab), so "similar color" behaves the way your eye expects.
...
- **Click** a swatch to set the active color. **Ctrl+click** adds it to the **merge queue** (orange outline).
- **Merge queue → C**: every queued color is repainted as the active color, across the frame or all frames (**V** toggles scope). This is the main palette-reduction move: queue the rare junk colors, pick their replacement, press C.
- **Shift+click** a swatch while a selection is active to remove that color from the selection.

## Requantize, filters, reduce

The left panel re-runs the import pipeline on demand:

- **Requantize (R)** re-matches pixels against a palette using the full [dithering & metric toolbox](Dithering-and-Color-Matching) (all 10 algorithms, 5 metrics, chroma boost, palette choice including "Tile colors" and the merge queue as a custom palette). The source can be the **original image** (crisp re-derivation; "Re-link file…" if the browser lost it) or the **current pixels**. Results preview first: **Enter/Y** commits, **Esc** cancels.
- **Filter (P)**: Smooth / Median / Sharpen (strength 0.5–3) / Posterize (2–8 levels), per frame or all frames. **Shift+P** cycles the type.
- **Reduce (K)** removes one color per press using the Rarest / Closest / Weighted strategy (**Shift+K** cycles). Watch the distinct-color count in the status bar.
...
| **Z** | Compression detail map | bright = expensive to compress, i.e. where your [byte budget](Codecs-and-Capacity) is going |
