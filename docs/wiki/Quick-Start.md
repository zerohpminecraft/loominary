...
| File | What it is |
|---|---|
| `loominary_state.json` | The encoded payload — the mod reads this |
...
> Already in-game with the file on disk? There's also a direct path: drop the image in `<gamedir>/loominary_data/` and run `/loominary import <filename>`. The web editor gives you more control (and is the only way to reduce colors, dither selectively, or encrypt), but for a quick simple image the command works.
...
1. Hold an **empty map** while standing on/near the platform, and use it — this snapshots the carpet colors into the map.
