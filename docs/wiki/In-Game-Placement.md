...
Load the schematic in Litematica (**M**) and position it flat on the ground. Two things matter enormously here: **where** (the map grid) and **what's north of it** (the noobline). Get both right before placing a single carpet.
...
A misaligned platform doesn't fail loudly — it shifts every pixel, and the decoder finds garbage. If a scan shows your art cut off at an edge and wrapping weirdly, alignment is the first suspect.
...
Map shading is relative to the **north**: a block renders *brighter* when it sits higher than its northern neighbor, *darker* when lower. Your carpets sit on top of the ground, so the platform's **top row is higher than the bare ground north of it** — and renders as a telltale strip of lighter pixels. Map-art veterans call it the **noobline**.

The fix: make the row of blocks **directly north of the platform's first row the same height as the carpet surface**. Either:
...
For **staircase** schematics (shade channel in use) this northern row isn't just cosmetic — the first data row is *read* relative to it, so it must sit at the schematic origin's y-level or the decode itself misreads.
...
Hang any map in an item frame, look at it, and run `/loominary preview` — the pending art is painted straight onto that framed map, **client-side only**: no blocks placed, nothing sent to the server, and only you see it. On a wall of frames it discovers the whole grid and paints every tile at once. `/loominary revert` (or `revert all`) puts the maps back exactly as they were.
