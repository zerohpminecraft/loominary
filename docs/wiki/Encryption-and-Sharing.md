...
Loominary art lives entirely in vanilla data (map NBT, world blocks), so there are three ways to share it:

- **In-world**: anyone running Loominary within 32 blocks of your framed, locked map sees the art. Nothing to distribute.
- **As files**: the export ZIP is the whole project. Someone with your `loominary_state.json` (placed in their `config/`) and schematics can rebuild the art anywhere, and the state JSON re-imports into the web editor, so they can remix it. Sending the ZIP is enough.
- **Out of the world**: capture any existing framed art with `/loominary import steal`; see [Archiving map art](Stealing-Map-Art).

Every payload embeds a **title** (≤64 chars) and **author** (≤16), plus a CRC32 integrity check and a random nonce, so authorship information stays attached to the art wherever it goes.
...
- Encryption covers the image and its metadata. The *existence* of Loominary-encoded data is still visible to any mod user; that is what the lock screen indicates.
